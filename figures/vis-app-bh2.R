...
bpath <- "../experiments/"
mlabs <- c("Lm", "c*'-'*probit", "c*'-'*logiti", "c*'-'*logit")
...
cfs <- data.frame(path = list.files(bpath, pattern = "cfx")) %>% 
  ...
  mutate(mod = factor(mod, levels = c("Lm", "BoxCox", "Colr", "Colrr"), 
                      labels = mlabs),
         xi = factor(xi, levels = c(0, 10^(0:4)), labels = c("-infinity", 0:4)),
         estimate = ifelse(mod %in% c("c*'-'*logit", "c*'-'*logitr"), 
                           -estimate, estimate))
...
lls <- data.frame(path = list.files(bpath, pattern = "logLik")) %>% 
  ...
  mutate(mod = factor(mod, levels = c("Lm", "BoxCox", "Colr", "Colrr"), 
                      labels = mlabs),
         xi = factor(xi, levels = c(0, 10^(0:4)), 
                     labels = paste0(c("-infinity", 0:4))))
...
p2 <- lls %>% 
  ...
  scale_color_viridis_d(name = parse(text = "log[10]~xi"), 
                        labels = parse(text = levels(cfs$xi))) +
  geom_path(aes(group = env), data = outliers, alpha = 0.5) +
...
ggsave("figures/vis-app-bh2-xi.pdf", height = 5, width = 7)
