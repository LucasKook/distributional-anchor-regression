...
p1 <- dat %>% 
  filter(ncl == Kwhich, n == "n==1000", ba != "M[X]==-1", model == "anchor") %>% 
...
p3 <- sum_dat2 %>% filter(shift != "do(A==0)", ba != "M[X]==-1", model == "anchor") %>% 
  ...
  facet_grid(shift ~ ba, labeller = label_parsed) +
  scale_color_viridis_d(name = expression(log[10]~xi), labels = parse(text = levels(sum_dat2$xi))) +
...
ggsave("figures/vis-iv2.eps", height = 5.5, width = 6.5)
